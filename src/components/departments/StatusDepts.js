...
class StatusDepts extends Component {
  constructor() {
    super()
    this.state = {
      status: 'all',
...
const mapStateToProps = (state, props) => {
  const id = props.id
  console.log('MSTP-DeptStatus', state, 'props-id', id, props)
  return {
    depts: state.depts,
    emps: state.emps,
    tickets: state.tickets.filter((ticket) => ticket.department === id),
...
export default connect(mapStateToProps)(StatusDepts)
